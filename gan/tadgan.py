...
from keras.layers import Layer, Dense, Conv1D, UpSampling1D, Cropping1D, \
...
class TadGAN(object):
    ...
    def _build(self):
        ...
        self.critic_x_model.compile(loss=[_wasserstein_loss, _wasserstein_loss, 'mse'], loss_weights=[1, 1, 10], optimizer=self.optimizer)
        ...
        self.critic_z_model.compile(loss=[_wasserstein_loss, _wasserstein_loss, 'mse'], loss_weights=[1, 1, 10], optimizer=self.optimizer)
        ...
        self.encoder_generator_model = Model(inputs=[x, z], outputs=[score_x_gen, score_z_enc, x_recon])
        self.encoder_generator_model.compile(loss=[_wasserstein_loss, _wasserstein_loss, 'mse'], loss_weights=[1, 1, 10], optimizer=self.optimizer)
        ...
        for epoch in range(epochs):
            ...
            for batch in range(n_batches):
                ...
                if (batch + 1) % encoder_generator_freq == 0:
                    encoder_generator_losses.append(self.encoder_generator_model.train_on_batch([x_batch, z_batch], [-ones, -ones, x_batch]))
    ...
    def save_model(self, save_path):
        """Save the trained TadGAN model to the specified path."""
        # Save the entire model
        tf.keras.models.save_model(self.encoder, save_path + '_encoder')
    ...
    def get_model(self, save_path):
        ...
        self.generator = tf.keras.models.load_model(save_path + '_generator')
        self.critic_x = tf.keras.models.load_model(save_path + '_critic_x')
        self.critic_z = tf.keras.models.load_model(save_path + '_critic_z')